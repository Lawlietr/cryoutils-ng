...
- **sudo（native）**：首啟 / 失敗密碼對話框；密碼只存 `e.Password`（記憶體），**不落磁碟**。**保留啟動問密碼**（用戶決策 2026-09-11：部分狀態讀取走 `sudo cat`，非 root 看不到，不做 pure lazy）；Phase 6.6 加 `authManager` 單對話框 flag 防疊開（啟動對話框 200ms 延遲 vs 操作對話框會導致雙問）。
- **scale（native，Phase 6.6）**：Fyne v2.7.4 無運行時 `SetScale()`，scale 於 app 初始化從 `FYNE_SCALE`/`Xft.dpi` 讀定 → 自適應用「啟動前自 re-exec」：`CRYOUTILS_FYNE_SCALE` 未設時讀 xrandr 解析度算 scale（clamp(寬/1280, 1.0, 2.0)，0.25 倍數）後帶 env var 重啟；`FYNE_SCALE` 手動覆蓋優先。
...
## Known Issues & Fixes
- **Native UI sudo 多次詢問**（2026-09-11 真機發現，Phase 6.6 修正中）：非 root 執行時，用戶回報啟動輸入一次後操作又被問一次。成因：(a) 啟動對話框延遲 200ms，用戶先點操作時 `runTask → ensure` 再開第二個對話框 → 疊開雙問；(b) `ui/fyneui/sudo_dialog.go` 為零呼叫死代碼，容易改錯路徑。修正：`authManager` 單對話框 flag + 刪死代碼；**啟動問密碼保留**（部分狀態讀取需 root）。另 Fyne v2.7.4 無 `SetScale()` API，4K scale 走 `FYNE_SCALE` env + 自 re-exec（Phase 6.6）。
...
- **Phase 6.6**: Native UI 4K 適配與 sudo 流程修正（2026-09-11 真機 feedback）— 依序：(1) sudo 雙問修正（保留啟動問 + `authManager` 防疊開 + 刪 `sudo_dialog.go` 死代碼）(2) FYNE_SCALE 自適應（自 re-exec）(3) 4K 排版（內容欄寬度上限置中、視窗跟隨螢幕、Status 雙欄、Card 化）(4) headless + 真機雙解析度驗證。見 `todo.md` Phase 6.6
